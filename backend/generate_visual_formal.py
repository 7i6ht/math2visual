    ...
    def render_svgs_from_data(self, output_file, resources_path, data):
        ...
        def handle_comparison(
            ...
            for i in range(2):
                ...
                try:
                    created, w, h = self.handle_all_except_comparison(operations_i,
                                    entities_i,
                                    svg_root,
                                    resources_path,
                                    result_i,
...
def extract_visual_language(text):
    ...
    return FormalVisualGenerator().extract_visual_language(text)

def parse_dsl(dsl_str):
    """Backward compatibility wrapper"""
    return FormalVisualGenerator().parse_dsl(dsl_str)

def render_svgs_from_data(output_file, resources_path, data):
    """Backward compatibility wrapper"""
    return FormalVisualGenerator().render_svgs_from_data(output_file, resources_path, data)
...
if __name__ == "__main__":
    ...
    generator = FormalVisualGenerator()
    data = generator.parse_dsl(visual_language[0])
    generator.render_svgs_from_data(output_path, svg_dataset, data)